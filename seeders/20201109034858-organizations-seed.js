  ...
  up: async (queryInterface) => {
    await queryInterface.sequelize.transaction(async (transaction) => {
      await queryInterface.sequelize.query(
        `
        insert into organization (organizationname, organizationtypeenum, recordcreatetimestamp, recordcreateuser_uuid, recordupdatetimestamp, recordupdateuser_uuid) select 'San Francisco Fire Department','EMS',now(),user_uuid, now(), user_uuid from batsuser where email = 'batsadmin@c4sf.me';
        insert into organization (organizationname, organizationtypeenum, recordcreatetimestamp, recordcreateuser_uuid, recordupdatetimestamp, recordupdateuser_uuid) select 'King American','EMS',now(),user_uuid, now(), user_uuid from batsuser where email = 'batsadmin@c4sf.me';
        insert into organization (organizationname, organizationtypeenum, recordcreatetimestamp, recordcreateuser_uuid, recordupdatetimestamp, recordupdateuser_uuid) select 'American Medical Response (AMR)','EMS',now(),user_uuid, now(), user_uuid from batsuser where email = 'batsadmin@c4sf.me';
