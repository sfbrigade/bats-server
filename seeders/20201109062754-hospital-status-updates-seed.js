  ...
  up: async (queryInterface) => {
    await queryInterface.sequelize.transaction(async (transaction) => {
      await queryInterface.sequelize.query(
        `
        insert into hospitalstatusupdate (hospital_uuid, updatedatetimelocal, edadminuser_uuid, openedbedcount, openpsychbedcount, divertstatusindicator, additionalserviceavailabilitynotes, recordcreatetimestamp, recordcreateuser_uuid, recordupdatetimestamp, recordupdateuser_uuid) select (select hospital_uuid from hospital where hospitalname='Kaiser SF'),now(),user_uuid, 9, 10, false, 'CT scanner broken', now(), user_uuid, now(), user_uuid from batsuser where email = 'batsadmin@c4sf.me';
        insert into hospitalstatusupdate (hospital_uuid, updatedatetimelocal, edadminuser_uuid, openedbedcount, openpsychbedcount, divertstatusindicator, recordcreatetimestamp, recordcreateuser_uuid, recordupdatetimestamp, recordupdateuser_uuid) select (select hospital_uuid from hospital where hospitalname='CPMC Davies'),now(),user_uuid, 88, 2, false, now(), user_uuid, now(), user_uuid from batsuser where email = 'batsadmin@c4sf.me';
        insert into hospitalstatusupdate (hospital_uuid, updatedatetimelocal, edadminuser_uuid, openedbedcount, openpsychbedcount, divertstatusindicator, recordcreatetimestamp, recordcreateuser_uuid, recordupdatetimestamp, recordupdateuser_uuid) select (select hospital_uuid from hospital where hospitalname='CPMC Van Ness'),now(),user_uuid, 0, 0, true, now(), user_uuid, now(), user_uuid from batsuser where email = 'batsadmin@c4sf.me';
        ...
        insert into hospitalstatusupdate (hospital_uuid, updatedatetimelocal, edadminuser_uuid, openedbedcount, openpsychbedcount, divertstatusindicator, recordcreatetimestamp, recordcreateuser_uuid, recordupdatetimestamp, recordupdateuser_uuid) select (select hospital_uuid from hospital where hospitalname='Chinese Hospital'),now(),user_uuid, 0, 0, true, now(), user_uuid, now(), user_uuid from batsuser where email = 'batsadmin@c4sf.me';
